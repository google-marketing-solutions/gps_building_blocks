...
class GoogleCloudStorageHook(gcs_hook.GoogleCloudStorageHook,
  ...
  def __init__(self, bucket: Text, prefix: Text = '',
               content_type: BlobContentTypes = BlobContentTypes.JSON) -> None:
    ...
    TODO(saraid): change prefix to a required field once datastoreOperator
    is removed.
    """
  ...
  def _parse_events_by_content_type(self, parsable_events: List[bytes]
    ...
    """
    if self.content_type == BlobContentTypes.CSV and parsable_events:
      return self._parse_events_as_csv(parsable_events)
    elif self.content_type == BlobContentTypes.JSON and parsable_events:
