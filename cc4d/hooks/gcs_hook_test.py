...
"""Tests for google3.third_party.gps_building_blocks.cc4d.hooks.gcs_hook."""
...
def get_expected(expected, content_type=gcs_hook.BlobContentTypes.JSON):
  ...
  Returns:
    Object containing parsed representation of the GCS Blob JSON.
  """
  events = b''.join(expected).splitlines()
  if content_type == gcs_hook.BlobContentTypes.JSON:
...
class JSONGoogleCloudStorageHookTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    with mock.patch.object(gcp_api_base_hook.GoogleCloudBaseHook, '__init__',
                           autospec=True):
      self.gcs_hook = gcs_hook.GoogleCloudStorageHook(bucket='bucket')
  ...
  def setUp(self):
    ...
    with mock.patch.object(gcp_api_base_hook.GoogleCloudBaseHook, '__init__',
      ...
      self.gcs_hook = gcs_hook.GoogleCloudStorageHook(
          bucket='bucket', content_type=gcs_hook.BlobContentTypes.CSV)
